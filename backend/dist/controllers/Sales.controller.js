...
export class SalesController {
    static async processSale(c) {
        ...
        try {
            ...
            const [venteResult] = await connection.execute('INSERT INTO VENTES (date_vente, matricule_employe, id_client) VALUES (NOW(), ?, ?)', [employeeId, clientId]);
            const venteId = venteResult.insertId;
            let totalAmount = 0;
            // 3. Process Items
            for (const item of items) {
                // Get current price and verify stock
                const [gadgets] = await connection.execute('SELECT prix_vente_gadget FROM GADGETS WHERE id_gadget = ?', [item.id_gadget]);
                if (gadgets.length === 0)
                    throw new Error(`Gadget ${item.id_gadget} not found`);
                const price = gadgets[0].prix_vente_gadget;
                // Check stock
                const [stocks] = await connection.execute('SELECT quantite_disponible FROM STOCKS WHERE id_gadget = ? FOR UPDATE', [item.id_gadget]);
                ...
                // Deduct Stock
                await connection.execute('UPDATE STOCKS SET quantite_disponible = quantite_disponible - ? WHERE id_gadget = ?', [item.quantity, item.id_gadget]);
                // Add to Concerner
                await connection.execute('INSERT INTO CONCERNER (id_gadget, id_vente, quantite_vendue, prix_unitaire_appliquer) VALUES (?, ?, ?, ?)', [item.id_gadget, venteId, item.quantity, price]);
                totalAmount += price * item.quantity;
            }
            // 4. Create Facture
            const invoiceNumber = `INV-${Date.now()}`;
            await connection.execute('INSERT INTO FACTURES (numero_facture, date_facture, montant_total, id_vente) VALUES (?, NOW(), ?, ?)', [invoiceNumber, totalAmount, venteId]);
